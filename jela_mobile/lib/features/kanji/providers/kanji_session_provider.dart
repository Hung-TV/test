...
class KanjiSessionProvider extends ChangeNotifier {
  final KanjiListService _service = KanjiListService();

  bool _isLoading = false;
  String? _errorMessage;
  List<KanjiQuizQuestion> _questions = [];
  int _currentIndex = 0;
  int? _selectedOptionIndex;

  // Track answers: kanjiId -> list of correct/incorrect values
  ...
  List<KanjiQuizQuestion> get questions => _questions;
  ...
  KanjiQuizQuestion? get currentQuestion =>
  ...
  void selectOption(int index) {
    ...
    _answersLog.putIfAbsent(question.kanjiId, () => []).add(isCorrect);
  ...
  Future<void> fetchExplanation() async {
    ...
    try {
      final correctChar = question.character;
      final selectedChar = question.options[_selectedOptionIndex!];
  ...
  List<Map<String, dynamic>> calculateQualityReviews() {
    ...
    _answersLog.forEach((kanjiId, list) {
      ...
      reviews.add({
        ...
        'totalCount': total,
        'character': _findKanjiCharacter(kanjiId),
      });
    });
    return reviews;
  }

  String _findKanjiCharacter(int kanjiId) {
    try {
      final match = _questions.firstWhere((q) => q.kanjiId == kanjiId);
